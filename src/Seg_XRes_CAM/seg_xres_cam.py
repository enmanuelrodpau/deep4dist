...
def vis_predict(image, model, preprocess_transform, DEVICE = 'cpu', mask = None, box = None, fig_name = None, vis = True):
    ...
    if mask is not None:
        input_tensor = input_tensor * mask.to(input_tensor.device)
    ...
    rect_image = cv2.rectangle(rect_image, (box[2], box[0]), (box[3], box[1]), (255,255,255), 10)
...
def rise_segmentation(masks, image, model, preprocess_transform, target = None, n_masks = None, box = None, DEVICE = 'cpu', vis = True, vis_skip = 1):
    ...
    for index, mask in tqdm(enumerate(masks[:n_masks])):
        ...
        input_tensor_1 = input_tensor * mask.to(input_tensor.device)
...
def rise_aggregated(image, masks, coef, fig_name = None, vis = True):
    ...
    image_np = image.detach().cpu().numpy()         # Now it's a NumPy array of shape (C, H, W)

    # Transpose to H×W×C if needed
    image_np = image_np.transpose(1, 2, 0)          # Now shape is (H, W, C)

    # Scale to [0,1] if it’s in [0,255]
    image_np = image_np / 255.0

    # Now call show_cam_on_image with NumPy arrays
    overlaid = show_cam_on_image(image_np, aggregated_mask / 255.0, use_rgb=True)
...
def seg_grad_cam(image, model, preprocess_transform, target=None, target_layer=None, box=None, DEVICE='cpu', method_index=0, fig_base_name=None, fig_name=None, vis_base=True, vis=True, negative_gradient=False, pool_size=None, pool_mode=np.max, reshape_transformer=False):
    # Decouple RGB visualization and model input
    if preprocess_transform is None:
        input_tensor = image.clone()  # Keep full 5-channel tensor for the model
        # Extract RGB channels (first 3) for visualization
        vis_image = image[:3].permute(1, 2, 0).cpu().numpy()
        max_, min_ = vis_image.max(), vis_image.min()
        vis_image = np.uint8(255 * (vis_image - min_) / (max_ - min_))
    else:
        input_tensor = preprocess_transform(image)  # Full tensor for the model
        vis_image = image[:3].permute(1, 2, 0).cpu().numpy()  # RGB for visualization

    # Ensure activations and gradients are lists
    actv, grad = [], []

    # Register hooks
    handle_1 = target_layer.register_forward_hook(lambda x, y, z: actv.append(z))
    handle_2 = target_layer.register_forward_hook(lambda x, y, z: save_grad(z, grad))
    ...
    if box is None:
        y_start, y_end, x_start, x_end = 0, vis_image.shape[0], 0, vis_image.shape[1]
    else:
        y_start, y_end, x_start, x_end = box[0], box[1], box[2], box[3]
    ...
    activations = actv[0][0].detach().cpu().numpy()
    gradients = grad[0][0].detach().cpu().numpy()
    ...
    # Compute Grad-CAM or Seg-XRes-CAM
    if method_index == 0:  # Seg-Grad-CAM
        coef = gradients.sum(axis=(1, 2))
        coef = coef[:, None, None]
        grayscale_cam = coef * activations
        grayscale_cam = grayscale_cam.sum(axis=0)
    elif method_index == 1:  # Seg-XRes-CAM
        if pool_size is not None:
            pooled = skimage.measure.block_reduce(gradients, (1, pool_size, pool_size), pool_mode)
            pooled = np.transpose(pooled, (1, 2, 0))
            gradients = skimage.transform.resize(pooled, (gradients.shape[1], gradients.shape[2]), order=0)
    ...
    # Normalize Grad-CAM
    grayscale_cam = np.maximum(grayscale_cam, 0)
    grayscale_cam = cv2.resize(grayscale_cam, (vis_image.shape[1], vis_image.shape[0]))
    max_, min_ = grayscale_cam.max(), grayscale_cam.min()
    grayscale_cam = np.uint8(255 * (grayscale_cam - min_) / (max_ - min_))
    grayscale_cam = grayscale_cam / 255.0

    # Overlay Grad-CAM on the RGB image
    overlaid = show_cam_on_image(vis_image / 255, grayscale_cam, use_rgb=True)
